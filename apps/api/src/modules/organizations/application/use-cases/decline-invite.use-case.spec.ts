...
import { DeclineInviteUseCase } from "./decline-invite.use-case.js";
...
describe("Decline Invite Use Case", () => {
    ...
    beforeEach(() => {
        ...
        sut = new DeclineInviteUseCase(membershipsRepository)
        vi.useFakeTimers()
    })
    ...
    it("should decline an membership invite from organization", async () => {
    ...
    it("should not decline a membership that does not exist", async () => {
        const inviter = User.create({ name: "Pedro Marques", email: "phmarkes@teste.com" })
        const org = Organization.create({ name: "Lumina" })
    ...
    it("should not decline another user's membership", async () => {
        const inviter = User.create({ name: "Pedro Marques", email: "phmarkes@teste.com" })
        const invitedUser = User.create({ name: "Joao Lucas", email: "joaolucas@teste.com" })
    ...
    it("should not decline an already declined membership", async () => {
          const inviter = User.create({ name: "Pedro Marques", email: "phmarkes@teste.com" })
        const invitedUser = User.create({ name: "Joao Lucas", email: "joaolucas@teste.com" })
